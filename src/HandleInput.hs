...
unlock_wrapper game_state code =
  let bit_list = pad (decimalBinary (hexDecimal (code !! 0) 0) (2 ^ 127)) [] 127 0
      state_values = extractStateValues (listArray (0, 127) bit_list) (s0_ game_state) (s1_ game_state) 0
...
set_keys game_state args = (Just game_state {s1_ = (s1_ game_state) {keys = EncodeStatus.setKeys [read (args !! 0), read (args !! 1), read (args !! 2), read (args !! 3), read (args !! 4), read (args !! 5)] 77}}, "set_keys succeeded.")
...
set_difficulty game_state args = (Just game_state {s1_ = (s1_ game_state) {difficulty = EncodeStatus.setDifficulty (read (args !! 0))}}, "set_difficulty succeeded.")
...
set_verbose_mode game_state args =
  ...
  else (Just game_state {s1_ = (s1_ game_state) {verbose_mode = intToBool mode}}, "set_verbose_mode succeeded.  Arguments passed were mode: " ++ (args !! 0))
...
keys_node = Comm_struct {dictionary_page = [], branches = Nothing, update_game_state = Just HandleInput.set_keys}

region_node = Comm_struct {dictionary_page = [], branches = Nothing, update_game_state = Just set_region}

difficulty_node = Comm_struct {dictionary_page = [], branches = Nothing, update_game_state = Just HandleInput.set_difficulty}

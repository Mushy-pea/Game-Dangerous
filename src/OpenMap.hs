...
openMap :: Int -> [Char] -> Int -> Int -> Int -> (Array (Int, Int, Int) Wall_grid, Array (Int, Int, Int) Floor_grid, Array (Int, Int, Int) Obj_grid)
openMap mode map_text u_limit v_limit w_limit =
  let fd = \limit -> (div (limit + 1) 2) - 1
      ...
      obj_grid = checkMapLayer 0 0 0 u_limit v_limit (emptyObjGrid u_limit v_limit w_limit // loadObjGrid mode (splitOn ", " (((splitOn "~" map_text), 62) !! 6)))
                               obj_grid_flag

...
loadObject [] = []
loadObject (x0:x1:x2:x3:x4:x5:x6:xs) = Obj_place {ident_ = read x0, u__ = 0, v__ = 0, w__ = 0, rotation = procInts [x1, x2, x3], rotate_ = loadGrid1 x4, phase = 0, texture__ = read x5, num_elem = read x6, obj_flag = 0} : loadObject xs

maybeObject :: Obj_place -> [Char]
maybeObject x =
  if ident_ x == 0 then "0, "
  else if obj_flag x > 7499 then throw Invalid_obj_flag
  else "1, " ++ (show (ident_ x)) ++ ", " ++ (show (u__  x)) ++ ", " ++ (show (v__ x)) ++ ", " ++ (show (w__ x)) ++ ", " ++ (showInts (rotation x)) ++ (showBool (rotate_ x)) ++ (show (phase x)) ++ ", " ++ (show (texture__ x)) ++ ", " ++ (show (num_elem x)) ++ ", " ++ (show (obj_flag x)) ++ ", "
...
padWalls :: Wall_grid -> Int -> Int -> Int -> [Char]
padWalls x u v w =
...
gridSetup1 (x0:x1:x2:x3:x4:x5:x6:xs) (y:ys) obj u v w v_max c =
  let ws = wallSetup x0
      w_grid = Wall_grid {u1 = (ws, 599) !! 0, u2 = (ws, 600) !! 1, v1 = (ws, 601) !! 2, v2 = (ws, 602) !! 3, u1_bound = fromIntegral u, u2_bound = fromIntegral (u + 1), v1_bound = fromIntegral v, v2_bound = fromIntegral (v + 1), w_level = fromIntegral w, wall_flag = [c, c + 1, c + 2, c + 3], texture = [read [x1], read [x2], read [x3], read [x4]], obj = mapObject x5 obj ((div u 2) * 2) ((div v 2) * 2) w y}
  in
  if v == v_max then w_grid : gridSetup1 xs ys obj (u + 1) 0 w v_max (c + 4)
  else w_grid : gridSetup1 xs ys obj u (v + 1) w v_max (c + 4)

gridSetup0 :: [Wall_grid] -> Int -> Int -> Int -> Int -> Int -> Int -> Seq Char -> Seq Char -> (Seq Char, Seq Char)
gridSetup0 (x:xs) u v w u_max v_max w_max acc0 acc1 =
  let w_grid = (showBool (u1 x)) ++ (showBool (u2 x)) ++ (showBool (v1 x)) ++ (showBool (v2 x)) ++ (show (u1_bound x)) ++ ", " ++ (show (u2_bound x)) ++ ", " ++ (show (v1_bound x)) ++ ", " ++ (show (v2_bound x)) ++ ", " ++ (show (w_level x)) ++ ", " ++ (showInts (wall_flag x)) ++ (showInts (texture x)) ++ maybeObject (fromMaybe d_obj (obj x))
  in
  if ((wall_flag x), 603) !! 3 > 119999 then throw Invalid_wall_flag
  else if u == u_max && v == v_max && w == w_max then (acc0 >< fromList (init_ w_grid ++ "~"), acc1 >< fromList (padWalls x u v w))
  else if u == u_max && v == v_max then gridSetup0 xs 0 0 (w + 1) u_max v_max w_max (acc0 >< fromList (init_ w_grid ++ "&")) (acc1 >< fromList (padWalls x u v w))
  else if v == v_max then gridSetup0 xs (u + 1) 0 w u_max v_max w_max (acc0 >< fromList (init_ w_grid ++ ":")) (acc1 >< fromList (padWalls x u v w))
  else gridSetup0 xs u (v + 1) w u_max v_max w_max (acc0 >< fromList w_grid) (acc1 >< fromList (padWalls x u v w))
...
procMap pre_map u_max v_max w_max =
  let next_c = div ((u_max + 1) * (v_max + 1)) 4
      ...
      floor = makeFloor0 (splitOn " " (map filter0 (concat [(pre_map, 604) !! w | w <- [(w_max + 2)..(w_max + 2 + w_max)]]))) 0 0 0 ((div (u_max + 1) 2) - 1) ((div (v_max + 1) 2) - 1) w_max
      w_grid = gridSetup0 (concat [gridSetup1 ((pre_map, 605) !! (w + 1)) (gridSetup2 (next_c * w) (next_c * w) 0 0 0 u_max v_max) (loadObject (splitOn ", " (filter1 ((pre_map, 606) !! 0)))) 0 0 w v_max ((u_max + 1) * (v_max + 1) * w * 4) | w <- [0..w_max]]) 0 0 0 u_max v_max w_max empty empty

...
data Model_ = Model_ {vertex_ :: [[Char]], tex_coord_ :: [[Char]], normal_ :: [[Char]], face_ :: [[Char]]} deriving Show
...
blank_model_ = Model_ {vertex_ = [], tex_coord_ = [], normal_ = [], face_ = []}
...
get_vert_ :: [[Char]] -> Int -> Int -> Model_ -> Model_
get_vert_ [] phase mode model = model
get_vert_ (x:xs) 0 mode model =
  if x == "v" then get_vert_ (drop 3 xs) 0 mode (model {vertex_ = (vertex_ model) ++ take 3 xs})
  else if x == "vt" then get_vert_ (drop 2 xs) 0 mode (model {tex_coord_ = (tex_coord_ model) ++ take 2 xs})
  else if x == "vn" then get_vert_ (drop 3 xs) 0 mode (model {normal_ = (normal_ model) ++ take 3 xs})
  else if x == "usemtl" then get_vert_ (drop 1 xs) 0 mode model
  else get_vert_ xs 1 mode model
get_vert_ (x:xs) 1 mode model =
  if x == "usemtl" then get_vert_ (drop 1 xs) 1 mode model
  else if x == "f" then get_vert_ xs 1 mode model
  else get_vert_ xs 1 mode (model {face_ = (face_ model) ++ [x]})
...
place_vert model mode vert_acc tex_acc norm_acc col_attrib c =
  ...
  else if mode == 0 then place_vert (model {face = drop 2 (face model)}) mode (vert_acc ++ concat_ 1 (take 3 (drop face_vert (vertex model))) ++ ", ") tex_acc (norm_acc ++ concat_ 0 (take 3 (drop face_norm (normal model))) ++ ", ") col_attrib (c + 1)
  else place_vert (model {face = drop 3 (face model)}) mode (vert_acc ++ concat_ 1 (take 3 (drop face_vert (vertex model))) ++ ", ") (tex_acc ++ concat_ 0 (take 2 (drop face_tex (tex_coord model))) ++ ", ") (norm_acc ++ concat_ 0 (take 3 (drop face_norm (normal model))) ++ ", ") col_attrib (c + 1)
